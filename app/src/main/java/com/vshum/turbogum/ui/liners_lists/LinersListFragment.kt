...
class LinersListFragment(var series: String) : Fragment(), LinersListContract.View,
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(requireActivity()).get(LinersListViewModel::class.java)
    ...
    override fun onSuccessList(linersList: ArrayList<Liner>) {
        ...
        when (series) {
            "Серия 1" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.series1List.clear(); viewModel.series1List.addAll(filtered)
                initRecyclerView(viewModel.series1List)
            }
            "Серия 2" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.series2List.clear(); viewModel.series2List.addAll(filtered)
                initRecyclerView(viewModel.series2List)
            }
            "Серия 3" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.series3List.clear(); viewModel.series3List.addAll(filtered)
                initRecyclerView(viewModel.series3List)
            }
            "Серия 4" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.series4List.clear(); viewModel.series4List.addAll(filtered)
                initRecyclerView(viewModel.series4List)
            }
            "Серия 5" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.series5List.clear(); viewModel.series5List.addAll(filtered)
                initRecyclerView(viewModel.series5List)
            }
            "Super 1" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.super1List.clear(); viewModel.super1List.addAll(filtered)
                initRecyclerView(viewModel.super1List)
            }
            "Super 2" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.super2List.clear(); viewModel.super2List.addAll(filtered)
                initRecyclerView(viewModel.super2List)
            }
            "Super 3" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.super3List.clear(); viewModel.super3List.addAll(filtered)
                initRecyclerView(viewModel.super3List)
            }
            "Sport 1" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.sport1List.clear(); viewModel.sport1List.addAll(filtered)
                initRecyclerView(viewModel.sport1List)
            }
            "Sport 2" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.sport2List.clear(); viewModel.sport2List.addAll(filtered)
                initRecyclerView(viewModel.sport2List)
            }
            "Classic 1" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.classic1List.clear(); viewModel.classic1List.addAll(filtered)
                initRecyclerView(viewModel.classic1List)
            }
            "Classic 2" -> {
                val filtered = linersListLocal.filter { it.series == series }
                viewModel.classic2List.clear(); viewModel.classic2List.addAll(filtered)
                initRecyclerView(viewModel.classic2List)
            }
        }
    }

    private fun initRecyclerView(list: ArrayList<Liner>) {
        val adapter = AdapterLinersList(list, this, appDao)
    ...
    override fun onAttach(context: Context) {
        ...
        // правильное имя из ServicesLocator — providerNavigatorParamLiners (с s)
